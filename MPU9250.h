...
float magCalibration[3] = {0, 0, 0}, magbias[3] = {205.407 ,29.535, -76.200}, magScale[3]={1.022, 0.962 ,1.018};  // Factory mag calibration and mag bias magbias[3] = {161.6494, 19.9466, -331.0574//151.926, -16.878, -331.278}
...
#define Kp 2.0f //2.0f * 5.0f these are the free parameters in the Mahony filter and fusion scheme, Kp for proportional feedback, Ki for integral
#define Ki 0.0f

float pitch, yaw, roll;
float deltat = 0.006f;//0.005f                             // integration interval for both filter schemes
    ...
    char readByte(uint8_t address, uint8_t subAddress)
{
    ...
    char checking(uint8_t address, uint8_t subAddress)
{
    char data[1]; // `data` will store the register data     
    char data_write[1];
    int result;
    data_write[0] = subAddress;
    i2c.write(address, data_write, 1, 1); // no stop
    result = i2c.read(address, data, 1, 0); 
    return result; 
}
...
// Configure FIFO to capture accelerometer and gyro data for bias calculation
  ...
  wait_ms(40); // accumulate 40 samples in 80 milliseconds = 480 bytes
...
// Configure the accelerometer for self-test
   ...
   wait_ms(3000); // Delay a while to let the device stabilize
 ...
 // Configure the gyro and accelerometer for normal operation
   ...
   wait_ms(1000); // Delay a while to let the device stabilize
